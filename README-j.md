...
# 状態
Freeze 前の細かな実装のチェックをしています。特に、gltf2 ライブラリへの対応はまだバグが潜在しています。
...
また、match コマンドで .obj ファイルを合致する際には若干おかしな結果が返ります。この場合、一度 tilt コマンドで obj ファイルを
画像へ変換した後、もう一度 bump コマンド mask, および obj コマンドで戻した結果を使用するとうまくいくことがあります。
ただし、この場合には片面である半分しか合致できないのでかなり煩雑になります。
