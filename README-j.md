...
Draft:
* 変形して制約を制限、緩和すると(元の問題とは異なってしまいますが)、{q_l} subset R^3 に対して, ||x_l-Sum p_k\*r_k+q_l||-&gt; min なる f:q_l -&gt; x_l : R^3 to R^3 に対して、Sum||x_l||, k -&gt; min (for all s, t in \[0, 1\]) s.t. x_l := q_l,0 \* t + (q_l,2 \* (1 - s) - q_l,1 \* s) \* (1 - t) となるため、QP になります。NP 完全かはおそらく先行があるはず。
