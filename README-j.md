...
# 状態
tools.cc の書き直しが必要です。また、新しくメタボールのボーンを計算できるか試行するか検討中です。
...
bump2 は擬似的に傾けた画像に対して、擬似的に 2 つのカメラで撮影したと仮定しています。
...
Draft:
* 変形して制約を制限、緩和すると(元の問題とは異なってしまいますが)、{q_l,k} を l 個目の面の k 個目の点と置いて、p_k, r_k を導出する関数として、Sum||p_k\*r_k+q_l,0\*t_l\*s_l+q_l,1\*(1-t_l)\*s_l+q_l,2\*(1-t_l)\*(1-s_l)|| -&gt; min, s.t. 0 &leq; s_l, t_l &leq; 1 となるため、QP のある程度幅が広い書き下しができます。もともと近似なので、QP Solver にかけるとある程度の範囲で解けることが期待されます。
