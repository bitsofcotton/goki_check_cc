...
Draft:
* 変形して制約を制限、緩和すると(元の問題とは異なってしまいますが)、||Sum p_k\*r_k+q_l,0\*t_l\*s_l+q_l,1\*(1-t_l)\*s_l+q_l,2\*(1-t_l)\*(1-s_l)|| -&gt; min, s.t. 0 &leq; s_l, t_l &leq; 1 となるため、QP のある程度幅が広い書き下しができます。もともと近似なので、QP Solver にかけるとある程度の範囲で解けることが期待されます。
