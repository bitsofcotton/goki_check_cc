...
template <typename T, typename U> P0<T,U>::P0(const int& range) {
  assert(1 < range);
  buf.resize(range);
...
template <typename T, typename U> const typename P0<T,U>::Mat& P0<T,U>::diff(const int& size, const bool& integrate) {
  ...
  d = (seed(size, true) * DD).template real<T>() * sqrt(sqrt(T(DD.rows() - 1)) / sqrt(nd * ni));
  i = (seed(size, true) * II).template real<T>() * sqrt(sqrt(T(II.rows() - 1)) / sqrt(nd * ni));
  if(integrate)
    return i;
  return d;
}
