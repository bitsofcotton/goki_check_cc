...
for line in argv[3:]:
  ...
  if(argv[2] == "col"):
    ...
    subprocess.call(["convert", "(", root + "-demosaic.ppm", "(", root + ".ppm", "-resize", str(s0 * 100) + "%", ")", "-compose", "subtract", "-composite", ")", "(", root + ".ppm", "-resize", str(s0 * 100) + "%", ")", "-compose", "add", "-composite", "-auto-level", root + "-demosaic.png"])
  elif(argv[2] == "habit"):
    if(len(bhabit) <= 0):
      bhabit = line
