...
if(argv[2] == "match"):
  ...
  if(ext1 == ".obj" or ext1 == ".gltf"):
    subprocess.call([argv[1], "match", "4", "20", "8", "6", "1", root0 + ".ppm", root0 + ".ppm", root0 + "-bump.ppm", argv[4], "match-" + root0 + "-" + argv[4]])
  elif(ext1 != ".ppm"):
    subprocess.call(["convert", argv[4], "-compress", "none", root1 + ".ppm"])
  subprocess.call([argv[1], "match", "4", "20", "8", "56", "15", root0 + ".ppm", root1 + ".ppm", root0 + "-bump.ppm", root1 + "-bump.ppm", "match-" + root0 + "-" + root1])
  exit(0)
...
for line in argv[3:]:
  ...
  if(argv[2] == "col"):
    ...
    subprocess.call(["convert", root + "-enl-0.ppm", "-sharpen", "1x1", "-resize", "75%", root + "-enl.png"])
  elif(argv[2] == "bump"):
    ...
    for s in range(0, s0):
      ...
      subprocess.call(["convert", root + "-" + str(s + 1) + "-0.ppm", "-sharpen", "1x1", "-resize", "75%", "-compress", "none", root + "-" + str(s + 1) + ".ppm"])
  elif(argv[2] == "habit"):
    if(len(bhabit) <= 0):
      bhabit = line
