...
if(len(argv) < 4):
  ...
  for line in argv[3:]:
    ...
    if(argv[2] == "collect" or argv[2] == "bump" or argv[2] == "illust" or argv[2] == "enlarge" or argv[2] == "sharpen"):
      subprocess.call([argv[1], argv[2], root + ".ppm", root + "-" + argv[2] + ".ppm", str(pixels), str(rot)])
    elif(argv[2] == "pextend"):
      subprocess.call([argv[1], argv[2], root + ".ppm", root + "-" + argv[2] + ".ppm", str(pixels), "1"])
      ...
      subprocess.call(["ffmpeg", "-loop", "1", "-i", root + "-btilt2-base-%d.ppm", "-framerate", "20", "-an", "-vf", "scale=trunc(iw/2)*2:trunc(ih/2)*2", "-vcodec", "libx264", "-pix_fmt", "yuv420p", "-t", "12", root + "-b2.mp4"])
    elif(argv[2] == "flicker"):
      for s in range(0, pixels):
      ...
      subprocess.call(["ffmpeg", "-loop", "1", "-i", root + "-flicker-base-%d.png", "-framerate", "6", "-an", "-vf", "scale=trunc(iw/2)*2:trunc(ih/2)*2", "-vcodec", "libx264", "-pix_fmt", "yuv420p", "-t", "12", root + "-ficker.mp4"])
    elif(argv[2] == "sbox"):
      for s in range(0, pixels):
      ...
      subprocess.call(["convert", root + "-bump.ppm", "-threshold", "100000", "-negate", "-threshold", "15500", "-negate", root + "-bump.ppm", "-compose", "minus", "-composite", root + "-nurie-base.png"])
