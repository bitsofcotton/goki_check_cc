...
for line in argv[3:]:
  ...
  if(argv[2] == "sample"):
    ...
    subprocess.call([argv[1], "reshape", str(pixels), root + "-bump.ppm", root + ".ppm", root + "-bump.ppm"])
    subprocess.call([argv[1], "reshape", str(pixels), root + ".ppm", root + "-bump.ppm", root + "-bump-rev.ppm"])
