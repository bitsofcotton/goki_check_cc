...
if(len(argv) < 4):
  ...
  nout = 8
  subprocess.call([argv[1], argv[2], "1", ".01", root + "-pose.txt", root + ".ppm", root + "-bump.ppm", str(nout), root + "-pose"])
  if(argv[2] == "poso"):
  ...
  root0, ext0 = os.path.splitext(argv[3])
  root1, ext1 = os.path.splitext(argv[4])
  subprocess.call([argv[1], argv[2], "1", ".01", root0 + "-bump.ppm", root1 + "-bump.ppm", root0 + "-pose.txt"])
else:
  for line in argv[3:]:
    try:
      pixels = int(line)
